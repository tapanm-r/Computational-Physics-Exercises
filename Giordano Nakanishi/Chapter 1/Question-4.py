...
if __name__ == "__main__":
    ...
    plt.figure(1)
    # Analytical Solution: N_A = N_A0 * exp{-t / tau_A}
    # N_B = N_B0 * exp{-t / tau_B} + N_A0 * (tau_B / (tau_B - tau_A)) * exp{-(t / tau_A + t / tau_B)} * (exp{t / tau_B} - exp{t / tau_A})
    t = np.linspace(0, tf, 1001, endpoint=True)
    N_A = N_A0 * np.exp(-t / tau_A)
    if tau_A != tau_B:
        N_B = N_B0 * np.exp(-t / tau_B) + N_A0 * (tau_B / (tau_A - tau_B))* (np.exp(- t / tau_A) - np.exp(t / tau_B))
    else:
        N_B = (N_B0 + N_A0 * t / tau_A) * np.exp(-t / tau_A)
    ...
    plt.title("Chain decay of Radioactive Nuclei")
    plt.grid()
    plt.legend(loc=1)

    fig = plt.figure(2)
    fig.suptitle("Chain Radioactive Decay")
    i = 1
    dt = 0.05
